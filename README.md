...
Latest release: 0.2.1 - cfssl multirootca 1.2.0 - [Changelog](CHANGELOG.md) | [Docker Hub](https://hub.docker.com/r/osixia/cfssl-multirootca/) 
...
- [Advanced User Guide](#advanced-user-guide)
	- [Extend osixia/cfssl-multirootca:0.2.1 image](#extend-osixiacfssl-multirootca021-image)
...
- Link your config file at run time to `/container/service/multirootca/assets/roots.conf` and the  corresponding needed files to `/container/service/multirootca/assets/files`:
      ...
      docker run --volume /data/my-roots.conf:/container/service/multirootca/assets/roots.conf --volume /data/files:/container/service/multirootca/assets/files --detach osixia/multirootca:0.2.0
...
Add `--env CFSSL_MUTLTIROOTCA_HTTPS=true` to run command then a certificate is created with the container hostname (it can be set by docker run --hostname option eg: pki.my-company.com).
	...
	docker run --env CFSSL_MUTLTIROOTCA_HTTPS=true --hostname pki.my-company.com --detach osixia/cfssl-multirootca:0.2.1
...
You can set your custom certificate at run time, add `--env CFSSL_MUTLTIROOTCA_HTTPS=true`, mount a directory containing those files to **/container/service/multirootca/assets/certs** and adjust their name with the following environment variables:
		...
		--detach osixia/cfssl-multirootca:0.2.1
...
To fix that run the container with `--copy-service` argument :
		...
		docker run [your options] osixia/cfssl-multirootca:0.2.1 --copy-service
...
Example command to run the container in `debug` mode:
	...
	docker run --detach osixia/cfssl-multirootca:0.2.1 --loglevel debug

See all command line options:

	docker run osixia/cfssl-multirootca:0.2.1 --help
...
### Extend osixia/cfssl-multirootca:0.2.1 image

If you need to add your custom TLS certificate, bootstrap config or environment files the easiest way is to extends this image.

Dockerfile example:

    FROM osixia/cfssl-multirootca:0.2.1
...
Run your image :
	...
	docker run -d billy-the-king/cfssl-multirootca:0.2.1
