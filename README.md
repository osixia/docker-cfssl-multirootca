...
To fix that run the container with `--copy-service` argument :
		...
		docker run [your options] osixia/cfssl-multirootca:0.1.0 --copy-service
...
Example command to run the container in `debug` mode:
	...
	docker run --detach osixia/cfssl-multirootca:0.1.0 --loglevel debug

See all command line options:

	docker run osixia/cfssl-multirootca:0.1.0 --help
