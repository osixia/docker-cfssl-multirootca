...
Latest release: 0.2.0 - cfssl multirootca 1.2.0 - [Changelog](CHANGELOG.md) | [Docker Hub](https://hub.docker.com/r/osixia/cfssl-multirootca/) 
...
- [Advanced User Guide](#advanced-user-guide)
	- [Extend osixia/cfssl-multirootca:0.2.0 image](#extend-osixiacfssl-multirootca020-image)
...
Add `--env CFSSL_MUTLTIROOTCA_HTTPS=true` to run command then a certificate is created with the container hostname (it can be set by docker run --hostname option eg: pki.my-company.com).
	...
	docker run --env CFSSL_MUTLTIROOTCA_HTTPS=true --hostname pki.my-company.com --detach osixia/cfssl-multirootca:0.2.0
...
You can set your custom certificate at run time, add `--env CFSSL_MUTLTIROOTCA_HTTPS=true`, mount a directory containing those files to **/container/service/multirootca/assets/certs** and adjust their name with the following environment variables:
		...
		--detach osixia/cfssl-multirootca:0.2.0
...
To fix that run the container with `--copy-service` argument :
		...
		docker run [your options] osixia/cfssl-multirootca:0.2.0 --copy-service
...
Example command to run the container in `debug` mode:
	...
	docker run --detach osixia/cfssl-multirootca:0.2.0 --loglevel debug

See all command line options:

	docker run osixia/cfssl-multirootca:0.2.0 --help
...
### Extend osixia/cfssl-multirootca:0.2.0 image

If you need to add your custom TLS certificate, bootstrap config or environment files the easiest way is to extends this image.

Dockerfile example:

    FROM osixia/cfssl-multirootca:0.2.0
...
Run your image :
	...
	docker run -d billy-the-king/cfssl-multirootca:0.2.0
